...
from app.config import colr, letter, width, height
...
class AppBody:
    ...
    def __init__(self, main_window):
        ...
        self.__left = self._config_left(body)
        self.__left.grid(row=0, column=0)

        self.__center = self._config_center(body)
        self.__center.grid(row=0, column=1)

        self.__right = self._config_right(body)
    ...
    def _config_right(self, body):
        ...
        self.__text_screen = Text(right, font=letter['principal'],
                                  bg=colr['grey'], fg=colr['white'], bd=3,
